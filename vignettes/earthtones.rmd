...
1. downloads a satellite image from google earth which in turn comes from a variety of sources, but many are from Landsat.
...
And now Fisher's irises are colored in a Bahama style.  However, actually data from two of the three iris species was collected by a botanist named Edgar Anderson from the [Gaspé Peninsula in Quebec](https://www.jstor.org/stable/2394164?seq=1#page_scan_tab_contents), so it might be better to use a color scheme from there for those two species.  
...
The sand-color is perhaps a bit sandier with the PAM approach.  This actually makes sense because the PAM method returns medoid rather than means so the outputs are guaranteed to be actual colors in the image.  For the k-mean method the mean of the colors may actually be color that is not itself present in the image.  
